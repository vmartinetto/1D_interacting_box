...
def Piers_Neck_War_Ens(x,r,vexact,density,accel,w,conv):
    ...
    while number > conv:
        ...
        #number=(np.abs(np.ones(Nx)-gdens/density)).max() 
        number = 4*np.pi*simps(r**2*np.abs(gdens-density))
        print('conv criterion: ', number)
    return vxc, valsg, vecsg
...
#Import density for N=1000 and x = np.linspace(0, 8, Nx)
densities=np.genfromtxt('../2data/2part_Nx1000_L8_sc0.1_20200400_swss_dens.dat')
densityGS = densities[:,0]
density1 = densities[:,1]
